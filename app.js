...
riot.tag2('banner', '<div class="{opts.styles.bannerContainer}"> <div class="{opts.styles.centerContainer}"> <div class="{opts.styles.leftPanel}"> <div class="{opts.styles.titleContainer}"> <h1>FOREVER BEATS</h1> <p>Melodic step sequencer</p> <a href="https://itunes.apple.com/us/app/forever-beats/id1163751920?ls=1&mt=8"> <img type="image/svg+xml" src="svg/app_store.svg"> </a> </div> </div> <div class="{opts.styles.rightPanel}"> <div class="{opts.styles.logoAlignmentContainer}"> <div class="{opts.styles.appImageContainer}"> <img src="img/app.png"> </div> <object type="image/svg+xml" data="svg/Audiobus-Badge-Input.svg"></object> </div> </div> </div> </div>', '', '', function(opts) {

	this.on('mount', function() {

	});
}, '{ }');
riot.tag2('buybuybuy', '<div class="{opts.styles.buybuybuyContainer}"> <div class="{opts.styles.buybuybuyCenterContainer}"> <h2>Download for iPad and iPad pro</h2> <img src="img/app2up.png"> <div class="{opts.styles.buybuybuyCenterSVG}"> <a href="https://itunes.apple.com/us/app/forever-beats/id1163751920?ls=1&mt=8"><img type="image/svg+xml" src="svg/app_store.svg"></a> </div> </div> </div>', '', '', function(opts) {
...
riot.tag2('faq', '<div class="{opts.styles.faqContainerCenter}"> <div class="{opts.styles.faqContainer}"> <h2 class="{opts.styles.faqTitle}">FAQ</h2> <h3 class="{opts.styles.question}">Where can I find the manual?</h3> <p class="{opts.styles.answer}">There is a manual available <a href="manual.pdf">here</a>.</p> <h3 class="{opts.styles.question}">Is this app getting Ableton Link?</h3> <p class="{opts.styles.answer}">Yes. Music is more fun when it is done with other people so Ableton Link is at the top of the feature list.</p> <h3 class="{opts.styles.question}">Does Forever Beats have MIDI clock?</h3> <p class="{opts.styles.answer}">Yes. Since version 1.0.8 you can sync Forever Beats with other software and hardware.</p> <h3 class="{opts.styles.question}">Can other apps see Forever Beats as a MIDI source?</h3> <p class="{opts.styles.answer}">Yes. Since version 1.0.7 Forever Beats has virtual MIDI. In your synth app of choice select "Forever Beats" as MIDI source and in Forever Beats set the "MIDI out" slider to active for each track you want to output MIDI data.</p> <h3 class="{opts.styles.question}">How is the MIDI working in Forever Beats?</h3> <p class="{opts.styles.answer}">Each track has its own MIDI output assignable to different channels. This can be set up in the MIDI settings modal, which each track has. There is a tutorial <a href="https://www.youtube.com/watch?v=RMbXYnahyxU">here</a>. By default it will output the note information for each track. You can also set each track to percussion mode for controlling <a href="https://www.youtube.com/watch?v=tavNwYtCGlY">drum machines</a>.</p> <h3 class="{opts.styles.question}">How many steps does the sequencer have?</h3> <p class="{opts.styles.answer}">The sequencer has 16 steps but you can change it for each track to have between 1 and 16 steps.</p> <h3 class="{opts.styles.question}">Something seems to be broken, what should I do?</h3> <p class="{opts.styles.answer}">If you are having problems or you seem to have run into something broken please report it in the <a href="https://groups.google.com/forum/#!forum/forever-beats">forum</a> and we will figure it out.</p> <h3 class="{opts.styles.question}">Are you implementating feature requests?</h3> <p class="{opts.styles.answer}">Yes. If you have some ideas feel free to share them in the <a href="https://groups.google.com/forum/#!forum/forever-beats">forum</a>. I am certainly listening but I can\'t promise to add everything. </p> </div> </div>', '', '', function(opts) {
...
riot.tag2('videos', '<div class="{opts.styles.videosContainer}"> <div class="{opts.styles.videosCenterContainer}"> <h2>VIDEO</h2> <iframe width="560" height="315" src="https://www.youtube.com/embed/FtAh6d5w2mA" frameborder="0" allowfullscreen></iframe> <h2>TUTORIAL</h2> <iframe width="560" height="315" src="https://www.youtube.com/embed/e2hnUryunzM" frameborder="0" allowfullscreen></iframe> </div> </div>', '', '', function(opts) {
